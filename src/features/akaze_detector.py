...
ALLOWED_PARAMS = {"threshold", "octaves", "octaveLayers", "diffusivity"}
...
class AKAZEDetector(BaseDetector):
    def __init__(self, **params):
        ...
        if bad_keys:
            raise ValueError(f"Unknown AKAZE parameters: {bad_keys}")
        self._detector = cv2.AKAZE_create(**params)
    ...
    def detect_and_compute(self, img: np.ndarray) -> FeatureResult:
        ...
        if work.dtype != np.uint8:
            ...
            import logging
            logging.getLogger(__name__).warning("Converted non-uint8 input to uint8 for AKAZE")
